...
use Veliu\RateManu\Application\Response\UserGroupCollectionResponse;
...
final readonly class GetMyGroupsAction
{
    #[OA\Response(
        ...
        content: new Model(type: UserGroupCollectionResponse::class)
    )]
    public function __invoke(UserInterface $user): JsonResponse
    {
        $domainUser = instance_of(User::class)->coerce($user);
        $response = UserGroupCollectionResponse::fromDomainCollection($domainUser->getGroups());
